...
public class ColorMRUBuilder {
  private List<ColorMRUModel.EG_ColorChoice> egColorChoice;

  public ColorMRUBuilder() {
  }

  public ColorMRUBuilder setEGColorChoice(List<ColorMRUModel.EG_ColorChoice> egColorChoice) {
  ...
  public static class EG_ColorChoice {
    ...
    public EG_ColorChoice from(ColorMRUModel.EG_ColorChoice value) {
      this.type = -1;
      this.value = null;
      if (value.isScrgbClr()) {
        this.type = 0;
    ...
    public ColorMRUModel.EG_ColorChoice build() {
      if (this.type == -1) return new ColorMRUModel.EG_ColorChoice();
      if (this.type == 0) return ColorMRUModel.EG_ColorChoice.newScrgbClr((ScRgbColorModel) this.value);
      if (this.type == 1) return ColorMRUModel.EG_ColorChoice.newSrgbClr((SRgbColorModel) this.value);
      if (this.type == 2) return ColorMRUModel.EG_ColorChoice.newHslClr((HslColorModel) this.value);
      if (this.type == 3) return ColorMRUModel.EG_ColorChoice.newSysClr((SystemColorModel) this.value);
      if (this.type == 4) return ColorMRUModel.EG_ColorChoice.newSchemeClr((SchemeColorModel) this.value);
      if (this.type == 5) return ColorMRUModel.EG_ColorChoice.newPrstClr((PresetColorModel) this.value);
      return null;
    }
  }
}
