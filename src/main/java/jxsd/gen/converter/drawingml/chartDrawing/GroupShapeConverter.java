...
public class GroupShapeConverter {
  ...
  public static GroupShapeModel fromDocx4j(CTGroupShape value) {
    ...
    List<GroupShapeModel.SpOrGrpSpOrGraphicFrame> spOrGrpSpOrGraphicFrame = value.getSpOrGrpSpOrGraphicFrame().stream().map(val -> {
      if (val instanceof CTShape) return CTGroupShape.SpOrGrpSpOrGraphicFrame.newSp(ShapeConverter.fromDocx4j((CTShape) val));
      if (val instanceof CTGroupShape) return CTGroupShape.SpOrGrpSpOrGraphicFrame.newGrpSp(GroupShapeConverter.fromDocx4j((CTGroupShape) val));
      if (val instanceof CTGraphicFrame) return CTGroupShape.SpOrGrpSpOrGraphicFrame.newGraphicFrame(GraphicFrameConverter.fromDocx4j((CTGraphicFrame) val));
      if (val instanceof CTConnector) return CTGroupShape.SpOrGrpSpOrGraphicFrame.newCxnSp(ConnectorConverter.fromDocx4j((CTConnector) val));
      if (val instanceof CTPicture) return CTGroupShape.SpOrGrpSpOrGraphicFrame.newPic(PictureConverter.fromDocx4j((CTPicture) val));
    ...
    public static CTGroupShape toDocx4j(GroupShapeModel value) {
      ...
      result.getSpOrGrpSpOrGraphicFrame().addAll(value.getSpOrGrpSpOrGraphicFrame().stream().map(val -> {
        if (val.isSp()) return GroupShapeConverter.toDocx4j(val.getSp());
        if (val.isGrpSp()) return GroupShapeConverter.toDocx4j(val.getGrpSp());
        if (val.isGraphicFrame()) return GroupShapeConverter.toDocx4j(val.getGraphicFrame());
        if (val.isCxnSp()) return GroupShapeConverter.toDocx4j(val.getCxnSp());
        if (val.isPic()) return GroupShapeConverter.toDocx4j(val.getPic());
        return null;
