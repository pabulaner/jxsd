...
public class BlipConverter {
  ...
  public static BlipModel fromDocx4j(CTBlip value) {
    ...
    List<BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor> alphaBiLevelOrAlphaCeilingOrAlphaFloor = value.getAlphaBiLevelOrAlphaCeilingOrAlphaFloor().stream().map(val -> {
      if (val instanceof CTAlphaBiLevelEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newAlphaBiLevel(AlphaBiLevelEffectConverter.fromDocx4j((CTAlphaBiLevelEffect) val));
      if (val instanceof CTAlphaCeilingEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newAlphaCeiling(AlphaCeilingEffectConverter.fromDocx4j((CTAlphaCeilingEffect) val));
      if (val instanceof CTAlphaFloorEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newAlphaFloor(AlphaFloorEffectConverter.fromDocx4j((CTAlphaFloorEffect) val));
      if (val instanceof CTAlphaInverseEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newAlphaInv(AlphaInverseEffectConverter.fromDocx4j((CTAlphaInverseEffect) val));
      if (val instanceof CTAlphaModulateEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newAlphaMod(AlphaModulateEffectConverter.fromDocx4j((CTAlphaModulateEffect) val));
      if (val instanceof CTAlphaModulateFixedEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newAlphaModFix(AlphaModulateFixedEffectConverter.fromDocx4j((CTAlphaModulateFixedEffect) val));
      if (val instanceof CTAlphaReplaceEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newAlphaRepl(AlphaReplaceEffectConverter.fromDocx4j((CTAlphaReplaceEffect) val));
      if (val instanceof CTBiLevelEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newBiLevel(BiLevelEffectConverter.fromDocx4j((CTBiLevelEffect) val));
      if (val instanceof CTBlurEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newBlur(BlurEffectConverter.fromDocx4j((CTBlurEffect) val));
      if (val instanceof CTColorChangeEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newClrChange(ColorChangeEffectConverter.fromDocx4j((CTColorChangeEffect) val));
      if (val instanceof CTColorReplaceEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newClrRepl(ColorReplaceEffectConverter.fromDocx4j((CTColorReplaceEffect) val));
      if (val instanceof CTDuotoneEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newDuotone(DuotoneEffectConverter.fromDocx4j((CTDuotoneEffect) val));
      if (val instanceof CTFillOverlayEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newFillOverlay(FillOverlayEffectConverter.fromDocx4j((CTFillOverlayEffect) val));
      if (val instanceof CTGrayscaleEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newGrayscl(GrayscaleEffectConverter.fromDocx4j((CTGrayscaleEffect) val));
      if (val instanceof CTHSLEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newHsl(HSLEffectConverter.fromDocx4j((CTHSLEffect) val));
      if (val instanceof CTLuminanceEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newLum(LuminanceEffectConverter.fromDocx4j((CTLuminanceEffect) val));
      if (val instanceof CTTintEffect) return BlipModel.AlphaBiLevelOrAlphaCeilingOrAlphaFloor.newTint(TintEffectConverter.fromDocx4j((CTTintEffect) val));
      return null;
