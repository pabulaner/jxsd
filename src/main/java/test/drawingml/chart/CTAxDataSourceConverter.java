...
public class CTAxDataSourceConverter
{
	...
	{
		if (value.getMultiLvlStrRef() != null) return CTAxDataSourceModel.createMultiLvlStrRef(
		CTMultiLvlStrRefConverter.fromDocx4j(value.getMultiLvlStrRef())
		);
		if (value.getNumRef() != null) return CTAxDataSourceModel.createNumRef(
		CTNumRefConverter.fromDocx4j(value.getNumRef())
		);
		if (value.getNumLit() != null) return CTAxDataSourceModel.createNumLit(
		CTNumDataConverter.fromDocx4j(value.getNumLit())
		);
		if (value.getStrRef() != null) return CTAxDataSourceModel.createStrRef(
		CTStrRefConverter.fromDocx4j(value.getStrRef())
		);
		if (value.getStrLit() != null) return CTAxDataSourceModel.createStrLit(
