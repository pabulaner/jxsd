...
public class EnumConverterParser implements SpecParser {
    ...
    public void parse(SpecContext ctx) {
        ...
        TypeName docx4jType = ParserUtil.convertType(specType, docx4jResolver);

        ///
        /// ---- FROM ----
        ///

        spec.getValues().forEach(value -> {
            value = new Name(value).toEnum();
            fromBuilder.addStatement("if (value == $T.$N) return $T.$N", docx4jType, value, modelType, value);
        });
        ...
        spec.getValues().forEach(value -> {
            value = new Name(value).toEnum();
            toBuilder.addStatement("if (value == $T.$N) return $T.$N", modelType, value, docx4jType, value);
        });
